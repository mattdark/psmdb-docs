...
* |PSMDB| packages now include ``mongosh`` instead of ``mongo`` CLI. If you :ref:`install Percona Server for MongoDB from tarballs <tarball>`, you must install ``mongosh`` from a separate tarball.
